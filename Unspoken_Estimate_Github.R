...
clean_swipe <- function(att_data, att_cols, att_recode){
  ...
  att_data$love <- as.numeric(dep_r == 2) # 0/1 if love
  att_data$hate <- as.numeric(dep_r == -2) # 0/1 if Hate
...
clean_conv <- function(conv_data, conv_cols){
  ...
  scale_time <- function(kdata, conv_cols){
    ...
    result <- data.frame(
      ...
      time_z_flip = time_z_flip,
      love = rep(0, nrow(kdata)),
      hate = rep(0, nrow(kdata)),
...
unspoken_est_one <- function(data_stan, data_model, control_code, stan_outname){
  ...
  if (min(meta_data$return_codes) == 0){
    ...
    checkconverge_export(draws_beta, vnames = colnames(data_stan$code_master), control_code$out_prefix, control_code$dir_run,
                         control_code$export_draws_betas, control_code$export_draws_means)
    if (data_model$est_att == 1){
      draws_thresh_base <- read_cmdstan_csv(meta_data$output_files, variables = "thresh_base", format = "draws_list")
      mychains <- lapply(draws_thresh_base$post_warmup_draws, function(x) do.call(cbind, x))
      fit_thresh_base <- plot_draws_df(mychains,  pdf_path = file.path(control_code$dir_run, paste0(control_code$out_prefix,"_thresh_base.pdf")))
      att_thresh <- fit_thresh_base$mean
    } else att_thresh <- NULL
    if (data_model$est_att == 1 & data_model$est_conv == 1){
      draws_beta_scale <- read_cmdstan_csv(meta_data$output_files, variables = "beta_scale", format = "draws_list")
      mychains <- lapply(draws_beta_scale$post_warmup_draws, function(x) do.call(cbind, x))
      fit_beta_scale <- plot_draws_df(mychains, pdf_path = file.path(control_code$dir_run, paste0(control_code$out_prefix, "_beta_scale.pdf")))
      write.table(fit_beta_scale, file = file.path(control_code$dir_run, paste0(control_code$out_prefix,"_beta_scale.csv")), sep = ",", na = ".", row.names = FALSE)
      beta_scale <- fit_beta_scale$mean
    } else beta_scale <- 1
    process_utilities_unspoken(data_stan, utilities, control_code$out_prefix, control_code$dir_run,
...
data_model <- c(data_model_base,
                list(
                  ...
                  love = data_stan$other_data$love,
                  hate = data_stan$other_data$hate,
